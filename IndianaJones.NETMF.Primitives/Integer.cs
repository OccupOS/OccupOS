...
namespace IndianaJones.NETMF.Integer
{
	...
	{
		public static Int16 Parse(string str)
		{
			Int16 result;
			if (TryParse(str, out result))
			{
		...
		public static Int16 Parse(string str, NumberStyle style)
		{
		...
		public static bool TryParse(string str, out Int16 result)
		{
			result = 0;
			ulong r;
			bool sign;
			if (Helper.TryParseUInt64Core(str, false, out r, out sign))
			{
				...
				else
				{
					...
					{
						result = (Int16)unchecked(-((Int16)r));
						return true;
					}
				}
			}
		...
		private static Int16 ParseHex(string str)
		{
			ulong result;
		...
		public static string ToHexString(Int16 value)
		{
			byte[] bytes = Microsoft.SPOT.Reflection.Serialize(value, typeof(Int16));
			return new string(ToHexStringArray(bytes));
		}
	...
	public static class Int32Extensions
	{
		...
		public static Int32 Parse(string str, NumberStyle style)
		{
			if (style == NumberStyle.Hexadecimal)
			{
				return ParseHex(str);
		...
		private static Int32 ParseHex(string str)
		{
			ulong result;
			if (TryParseHex(str, out result))
			{
	...
	{
		public static UInt32 Parse(string str)
		{
			UInt32 result;
			if (TryParse(str, out result))
			{
		...
		public static bool TryParse(string str, out UInt32 result)
		{
			bool sign;
			ulong tmp;

			bool bresult = Helper.TryParseUInt64Core(str, false, out tmp, out sign);
			result = (UInt32)tmp;
		...
		private static UInt32 ParseHex(string str)
		{
			UInt32 result;
			if (TryParseHex(str, out result))
			{
		...
		private static bool TryParseHex(string str, out UInt32 result)
		{
			bool sign;
			ulong tmp;

			bool bresult = Helper.TryParseUInt64Core(str, true, out tmp, out sign);
			result = (UInt32)tmp;
	...
	public static class CharExtensions
	{
		...
		public static bool TryParse(char character, out int result)
		{
			result = 0;
			switch (character)
			{
	...
	/// <summary>
	/// Supplement to the functions missing in .NET MF Guid object.
	/// </summary>
	public static class GuidExtensions
	{
		/// <summary>
		/// Creates a Guid from a string input.
		/// </summary>
		/// <param name="s"></param>
		/// <returns></returns>
		public static Guid FromString(string s)
		{
			...
			b[0] = ConvertTwoCharsToByte(s[6], s[7]);
			b[1] = ConvertTwoCharsToByte(s[4], s[5]);
			b[2] = ConvertTwoCharsToByte(s[2], s[3]);
			b[3] = ConvertTwoCharsToByte(s[0], s[1]);
			// -
			b[4] = ConvertTwoCharsToByte(s[11], s[12]);
			b[5] = ConvertTwoCharsToByte(s[9], s[10]);
			// -
			b[6] = ConvertTwoCharsToByte(s[16], s[17]);
			b[7] = ConvertTwoCharsToByte(s[14], s[15]);
			// -
			b[8] = ConvertTwoCharsToByte(s[19], s[20]);
			b[9] = ConvertTwoCharsToByte(s[21], s[22]);
			// -
			b[10] = ConvertTwoCharsToByte(s[24], s[25]);
			b[11] = ConvertTwoCharsToByte(s[26], s[27]);
			b[12] = ConvertTwoCharsToByte(s[28], s[29]);
			b[13] = ConvertTwoCharsToByte(s[30], s[31]);
			b[14] = ConvertTwoCharsToByte(s[32], s[33]);
			b[15] = ConvertTwoCharsToByte(s[34], s[35]);

			return new Guid(b);
		}

		/// <summary>
		/// Helper function that combines two characters into a single byte value.
		/// </summary>
		/// <param name="c1"></param>
		/// <param name="c2"></param>
		/// <returns></returns>
		private static byte ConvertTwoCharsToByte(char c1, char c2)
		{
			byte b1;
			byte b2;
			byte result;
